...
Zotero.ZotCard.Cards = Object.assign(Zotero.ZotCard.Cards, {
	...
	newCard: function (collection, item, type, text) {
		var pref = this.initPrefs(type);
		if (!pref || !pref.card) {
			Zotero.ZotCard.Messages.warning(Zotero.ZotCard.L10ns.getString('zotcard-pleaseconfigure', {name: type}));
			return;
		}
	...
	newCardWithTemplate: function (collection, item, template, text) {
		try {
			...
			// 0: 周日开始
			// 1: 周一开始
			let startOfWeek = Zotero.ZotCard.Prefs.get('startOfWeek', 0);
